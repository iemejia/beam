...
/**
 * A {@link RangeTracker} for non-negative positions of type {@link BigInteger}.
 */
public class BigIntegerRangeTracker implements RangeTracker<BigInteger> {
  private static final Logger LOG = LoggerFactory.getLogger(BigIntegerRangeTracker.class);

  private static BigInteger MINUS_ONE = BigInteger.valueOf(-1L);

  private BigInteger startOffset;
  private BigInteger stopOffset;
  private BigInteger lastRecordStart = MINUS_ONE;
  private BigInteger offsetOfLastSplitPoint = MINUS_ONE;
  private BigInteger splitPointsSeen = BigInteger.ZERO;
  ...
  public BigIntegerRangeTracker(BigInteger startOffset, BigInteger stopOffset) {
  ...
  public synchronized boolean isStarted() {
    ...
    return (!offsetOfLastSplitPoint.equals(MINUS_ONE)) || done;
  }
  ...
  @Override
  public synchronized BigInteger getStartPosition() {
    return startOffset;
  }

  @Override
  public synchronized BigInteger getStopPosition() {
    return stopOffset;
  }

  @Override
  public boolean tryReturnRecordAt(boolean isAtSplitPoint, BigInteger recordStart) {
  ...
  @Override
  public synchronized boolean trySplitAtPosition(BigInteger splitOffset) {
   ...
   */
  @VisibleForTesting
  BigIntegerRangeTracker copy() {
    synchronized (this) {
      BigIntegerRangeTracker res = new BigIntegerRangeTracker();

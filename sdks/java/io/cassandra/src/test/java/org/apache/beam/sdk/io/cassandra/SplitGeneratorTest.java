...
public final class SplitGeneratorTest {
  ...
  public void testGenerateSegments() {
    ...
    List<List<BigIntegerRange>> segments = generator.generateSplits(10, tokens);

    assertEquals(12, segments.size());
    assertEquals(
        "[[0, 1), [1, 14178431955039102644307275309657008811)]", segments.get(0).toString());
    assertEquals(
    ...
    assertEquals(12, segments.size());
    assertEquals(
        "[[5, 6), [6, 14178431955039102644307275309657008815)]", segments.get(0).toString());
    assertEquals(
        "[[70892159775195513221536376548285044053, 85070591730234615865843651857942052863)]",
        segments.get(5).toString());
    assertEquals(
  ...
  public void testRotatedRing() {
    ...
    SplitGenerator generator = new SplitGenerator("foo.bar.RandomPartitioner");
    List<List<BigIntegerRange>> segments = generator.generateSplits(5, tokens);

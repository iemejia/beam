...
public class ParquetTableFilterTest {
  ...
  public static Collection<Object[]> data() {
    return Arrays.asList(
        new Object[][] {
          {"select * from TEST where key = '100'", true},
//          {"select * from TEST where key >= 'key2'", true},
//          {"select * from TEST where key >= 100 AND key <= 1000", true},
//          {"select * from TEST where key IN {'100', '101'}", true},
//          {"select * from TEST where key LIKE '^key[123]'", false},
//          {"select * from TEST where key = name + key = '10'", false}
        });
  }
  ...
  public void testIsSupported() {
    ...
    ParquetTableFilter filter =
        new ParquetTableFilter(
            ((BeamCalcRel) beamRelNode).getProgram().split().right, BASIC_SCHEMA);
  ...
  public void testFilterPredicates() {
    ...
    ParquetTableFilter filter =
            new ParquetTableFilter(
                    ((BeamCalcRel) beamRelNode).getProgram().split().right, BASIC_SCHEMA);
    FilterPredicate filterPredicate = filter.buildPredicate(new ArrayList<>());
